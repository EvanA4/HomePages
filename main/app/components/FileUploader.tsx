    ...
    function handleUpload(files: FileList) {
        ...
        for (let i = 0; i < files.length; ++i) {
            ...
            if (
                ...
                (partitions.length > 1 && props.allowedTypes.includes(partitions[partitions.length - 1]))
            ) {
                uploads.push({
                    name: files[i].name,
                    size: files[i].size,
        ...
        >
            ...
            <div
                className={
                    ...
                    + (isDragging ? 'bg-[#2e2e2e] border-neutral-400' : 'bg-neutral-800 hover:bg-[#2e2e2e] border-neutral-600 hover:border-neutral-400')
                }
            >
                <Image
                    height={80}
                    ...
                    alt="upload icon"
                    src="/svgs/upload.svg"
                    className={isDragging ? 'opacity-100' : 'opacity-70 hover:opacity-100'}
                    draggable={false}
                    priority
                />
                ...
                >
                    {uploaded.map((upload, idx) =>
                        <div
                            key={idx}
                            className='p-3 bg-neutral-700 flex justify-between gap-5 rounded-lg'
                        >
                            <p><b>{upload.name}</b></p>
                            <p className='text-neutral-300'>{prettySize(upload.size)}</p>
                        </div>
                    )}
                </div>
            </div>
